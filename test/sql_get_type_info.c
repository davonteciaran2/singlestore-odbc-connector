...
void get_conn_string(char* c, char* add_params) {
    sprintf((char *) c, "DRIVER=%s;SERVER=%s;UID=%s;PASSWORD=%s;DATABASE=%s;%s;NO_SSPS=%d;%s",
            my_drivername, my_servername, my_uid, my_pwd, my_schema, ma_strport, NoSsps, add_params);
}
...
ODBC_TEST(t_sqlgettypeinfo2) {
    ...
    get_conn_string((char *) conn, add_connstr);
