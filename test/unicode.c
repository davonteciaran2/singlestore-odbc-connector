...
ODBC_TEST(sqlprimarykeys)
{
  ...
  CHECK_STMT_RC(hstmt1,
          SQLExecDirectW(hstmt1,
                         W(ServerNotOlderThan(Connection, 7, 3, 0) ? L"CREATE ROWSTORE TABLE t_a\x00e3g (a INT PRIMARY KEY)" :
                                                                         L"CREATE TABLE t_a\x00e3g (a INT PRIMARY KEY)"),
                         SQL_NTS));
